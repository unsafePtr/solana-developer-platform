...
You provision custody wallets up front (see [Set Up Wallets](/docs/guides/setup-wallets)) and reference them by their SDP `wallet_id` (`wal_…`). Transfer endpoints expect the **source** as a custody wallet ID — the wallet is resolved server-side from the caller's custody-wallet list — and the **destination** as the on-chain Solana address of the recipient. Ramp quote endpoints use the same shape for every provider: `destinationWallet` (onramp) or `sourceWallet` (offramp) is a custody wallet you control, passed as its wallet ID or its Solana address. Provider-specific identifiers (customer id, payout account) are resolved server-side from the **Counterparty**'s Provider Requirements, not passed on the request — see [Ramps](/docs/payments/ramps).
